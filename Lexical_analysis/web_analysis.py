...
def collect_constants_and_variables(code):
    tokens = re.findall(r'\b[a-zA-Z][a-zA-Z0-9]{0,9}\b|<=|>=|==|\+\+|\d+\b|\S', code.lower())
    for token in tokens:
        if re.match(r'\d+\b', token) and token not in CONSTVAL:
...
def lexical_analysis(code):
    tokens = re.findall(r'\b[a-zA-Z][a-zA-Z0-9]{0,9}\b|<=|>=|==|\+\+|\d+\w*|\S', code.lower())
    symbol_table = []
    used_vars = set()
    for token in tokens:
        if token == 'over':
