...
def to_doted_grammar(grammar):
    doted_grammar = []
    for production in grammar:
        left, right = production.split(" -> ")
        doted_grammar.append(left + " -> ." + right)
...
VN = ['E', 'T', 'F']
DFA = 
doted_grammar = to_doted_grammar(grammar)

def to_int_mapping(symbols):
    return {symbol: i for i, symbol in enumerate(symbols)}

VT2Int = to_int_mapping(VT)
VN2Int = to_int_mapping(VN)


action, goto = get_LR_table(grammar, doted_grammar, VT2Int, VN2Int, VN, VT, DFA, DFA_node)
