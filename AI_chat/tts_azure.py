...
def synthesize_speech(text, voice):
    ...
    output_audio_filename = "./output/" + text[:20] + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + ".wav"
...
def main():
    ...
    iface = gr.Interface(fn=process, 
                         inputs=[gr.Textbox(lines=2, placeholder="Enter your question here..."), gr.Dropdown(choices=models, label="Model"), gr.Dropdown(choices=get_voices(), label="Voice")], 
                         outputs=["text", "audio"], 
                         title="AI对话机器人",
